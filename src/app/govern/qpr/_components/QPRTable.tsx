          ...
          <thead>
            <tr className="border-b border-border bg-muted">
              <th className="px-4 py-3 w-8">
                <input type="checkbox" className="accent-primary w-3.5 h-3.5" onChange={onToggleAll}
            ...
            {visible.map(row => {
              ...
              return (
                <tr key={row.id} className={`border-b border-border last:border-0 transition-colors duration-150 ${isMissed ? 'bg-status-risk/5 hover:bg-status-risk/10' : ''} ${selected.has(row.id) ? 'bg-primary/5' : ''}`}>
                  <td className="px-4 py-3">{isMissed && <input type="checkbox" className="accent-primary w-3.5 h-3.5" checked={selected.has(row.id)} onChange={() => onToggleRow(row.id)} />}</td>
            ...
            <div key={row.id} className={`border rounded-sm p-3 ${isMissed ? 'border-status-risk/30 bg-status-risk/5' : 'border-border bg-card'} ${selected.has(row.id) ? 'border-primary bg-primary/5' : ''}`}>
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="flex items-start gap-2">
                  {isMissed && <input type="checkbox" className="accent-primary w-3.5 h-3.5 mt-0.5" checked={selected.has(row.id)} onChange={() => onToggleRow(row.id)} />}
                  <div>
