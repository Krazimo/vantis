      ...
      <div className="flex items-start justify-between gap-6">
        <div className="flex-1">
          <div className="text-sm text-off-white mb-1">
            Enable Demo Mode
            {demoMode && (
              <span className="ml-2 text-[10px] bg-gold/15 text-gold border border-gold/30 px-2 py-0.5 rounded-sm font-bold">DEMO</span>
            )}
          </div>
        ...
        <button onClick={onToggle}
          className={`relative w-10 h-5 rounded-full transition-colors duration-200 shrink-0 ${demoMode ? 'bg-gold' : 'bg-surface2 border border-border'}`}>
