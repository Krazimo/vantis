          ...
          <div className="bg-surface border border-border rounded-sm p-6 max-w-md w-full" onClick={e => e.stopPropagation()}>
            ...
            <div className="border-l-4 border-green bg-green/5 px-4 py-3 rounded-sm mb-5">
              <div className="flex items-start gap-2">
                ...
                <span className="text-green text-xs leading-relaxed">All five database checks passed or resolved. This application is cleared for registration.</span>
              </div>
            </div>
            <div className="flex gap-3">
              <button onClick={onConfirmApprove} className="flex-1 bg-gold text-background text-sm font-semibold py-2.5 rounded-sm hover:bg-gold-light transition-colors duration-150">Confirm Approval</button>
              <button onClick={onClose} className="flex-1 border border-border text-gray text-sm py-2.5 rounded-sm hover:text-off-white transition-colors duration-150">Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      {rejectModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-4" onClick={onClose}>
          <div className="bg-surface border border-border rounded-sm p-6 max-w-sm w-full" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-3">
              <button onClick={onConfirmReject} className="flex-1 bg-red/10 border border-red/40 text-red text-sm py-2.5 rounded-sm hover:bg-red/20 transition-colors duration-150 font-medium">Confirm Rejection</button>
      ...
      {conditionsModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-4" onClick={onClose}>
          <div className="bg-surface border border-border rounded-sm p-6 max-w-sm w-full" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-3">
              <button onClick={onConfirmConditions} className="flex-1 bg-amber/10 border border-amber/40 text-amber text-sm py-2.5 rounded-sm hover:bg-amber/20 transition-colors duration-150 font-medium">Issue Conditional Approval</button>
              <button onClick={onClose} className="flex-1 bg-surface2 border border-border text-gray text-sm py-2.5 rounded-sm hover:text-off-white transition-colors duration-150">Cancel</button>
            </div>
          </div>
        </div>
