    ...
    <main className="min-h-screen bg-background flex flex-col">
      <header className="flex items-center justify-between px-5 sm:px-10 py-4 border-b border-border">
        <div className="flex items-center gap-2.5">
          ...
          <span className="hidden sm:inline text-muted-foreground text-[10px] uppercase tracking-[0.15em] ml-2 border-l border-border pl-3">by Orianode</span>
        </div>
      ...
      <section className="flex-1 flex flex-col items-center justify-center px-5 pt-16 pb-12">
        <div className="flex items-center gap-2 mb-6">
          ...
          <span className="text-[10px] text-primary/50 uppercase tracking-[0.2em]">{tx.tagline}</span>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 w-full max-w-xl mt-12">
          {tx.stats.map((s, i) => (
            <div key={i} className="bg-card border border-border rounded-sm p-4 sm:p-5 text-center">
              ...
              <div className="text-[10px] text-muted-foreground uppercase tracking-[0.12em] mt-1.5 leading-tight">{s.label}</div>
            </div>
          ))}
        </div>
      ...
      <footer className="py-4 text-center border-t border-border px-4">
        <span className="text-[10px] text-muted-foreground uppercase tracking-[0.12em]">{tx.poweredBy}</span>
      </footer>
    </main>
  )
}
