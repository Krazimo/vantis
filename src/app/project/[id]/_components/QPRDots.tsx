  ...
  return (
    <div className="bg-card border border-border rounded-sm p-5">
        ...
        {quarters.map(q => {
          ...
          return (
            <div key={q} className="flex flex-col items-center gap-2 flex-1 relative z-10">
              ...
              />
              <span className="font-mono text-muted-foreground text-[9px] sm:text-xs text-center leading-tight whitespace-nowrap">
                {q.replace(' ', '\n')}
              </span>
            </div>
          )
      ...
      <div className="flex items-center gap-4 pt-3 border-t border-border flex-wrap">
        {[
          { color: 'bg-status-compliant',    label: 'Filed on time' },
          { color: 'bg-status-caution',    label: 'Filed late' },
          { color: 'bg-status-risk',      label: 'Missed' },
          { color: 'bg-muted/30',  label: 'Pre-registration' },
        ].map(({ color, label }) => (
